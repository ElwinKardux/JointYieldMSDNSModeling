...
nominal_Inflation_Rate = 0.01 # <- Best Estimate Inflation
nominal_forward_rate = -0.05 # <- Short term forward Rate
real_Inflation_Rate = 0.01 # <- Same as BEI?
real_forward_rate = 0.01-0.02 # <- forward rate - inflation
...
sigma_r = 0.00184         # <- Historical data based on germany yield curve

rho_nr = 0.175            # <- Using historical data
rho_rI = -0.065           # <- Using historical data
rho_In = -0.065           # <- Using historical data
...
P_n = utils.comp_Pn(t, T[1], NominalMarketPrice_T, NominalMarketPrice_t, sigma_n, a_n)
...
for i in range(1,len(T)-1):
    ...
    T_i = T[i]
    T_imin = T[i-1]
    NominalMarketPrice_T_imin = NominalMarketPrice_T[i-1]
    NominalMarketPrice_T_i = NominalMarketPrice_T[i]
    RealMarketPrice_T_imin = NominalMarketPrice_T[i-1]
    RealMarketPrice_T_i = NominalMarketPrice_T[i]
    phi_i = 1 #?
    Notional = cf_0[i-1]
    Notional_2 = cf_0[i]
    # Notional, Notional_2 = 1,1 # for ease
    P_n[i] = utils.comp_Pn(t, T_i, NominalMarketPrice_T[i], NominalMarketPrice_t, sigma_n, a_n)
    
    m_[i] = utils.comp_m_i(t,T_imin, T_i, NominalMarketPrice_T_imin, NominalMarketPrice_T_i, NominalMarketPrice_t, RealMarketPrice_T_imin, RealMarketPrice_T_i, RealMarketPrice_t,  a_n, a_r, sigma_n, sigma_I, sigma_r, rho_nr, rho_rI)
...
# # Cap + Floor
# =============================================================================
prices_caplets_long0_disc = utils.discount_values_simple(prices_caplets_long0, 0.02)
prices_caplets_short3_disc = utils.discount_values_simple(prices_caplets_short3, 0.02)
...
# # Recovery Clause
# =============================================================================
prices_caplets_long3_disc = utils.discount_values_simple(prices_caplets_long3, 0.02)
prices_caplets_short6_disc = utils.discount_values_simple(prices_caplets_short6, 0.02)
...
d = {'Year': np.arange(1,66), 'cashflows': cf_1[1:], 'Infl_clause': Final_pricelets, 'Cap_Floor': BS1_caplets, 'Recovery_clause': BS2_caplets, 'Catch-up clause':Floorlets}
...
with pd.ExcelWriter('Liabilities.xlsx') as writer:  
