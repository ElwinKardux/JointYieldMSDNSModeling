function [output, LL, xi_0, xi_1, P_0, P_1, xi00_pred, xi01_pred, xi10_pred, xi11_pred, RMSE, AIC, BIC, xi_smooth, pr_sm0] = NegLogLikeLambda(param_vector, y252)
%==========================================================================
%      As in ATSE:   
...
%==========================================================================
ft     = param_vector(1:4);
lambda_0 = param_vector(5);
alpha  = param_vector(6);
S0_Pr  = param_vector(7);                  % Pr[St=1|St-1=1]
S1_Pr  = param_vector(8);                  % Pr[St=0|St-1=0]
sigmas = param_vector(9:12);               % 
mu_states = param_vector(13:16);
lambda_1 = param_vector(17);
%mu_states = param_vector(17:20);

[F, H_0, R, Q, dt, ct] = DNS(ft, mu_states, lambda_0, alpha, sigmas);
[~, H_1, ~, ~, ~] = DNS(ft, mu_states, lambda_1, alpha, sigmas);
...
% xi_prev0 = (eye(4) - F) \ mu_states';  %inv(eye(4) - F) * mu_state0; % Initialize at the unconditional mean
% xi_prev1 = xi_prev0; %(eye(4) - F) \ mu_states';  %inv(eye(4) - F) * mu_state1;
...
PR_TT0M=zeros(T,1);  %@will store Pr[S_t=0|Y_t]@ necessary for smoothing
PR_TL0M=zeros(T,1);  %Pr[S_t=0|Y_{t-1}]
% Start iteration
for it = 1:T
    
    PR_TL0M(it)= S0_Pr*Prob0 + (1-S1_Pr)*Prob1;%
    %  @===================================================================@
    %  @=======================Kalman Filter===============================@
    %  @===================================================================@
    
    %  @=======================PREDICTION==================================@
    xi_pred00 = dt + F * xi_prev0;  % When S_{t-1}=0, S_{t}=0 -> beta_{t|t-1}^{0,0}
    ...
    %[12x1]     
    forcast_error00= y252(it,:)'- H_0 * xi_pred00 -ct;     % Conditional forecast error: eta_{t|t-1}^{0,0}
    forcast_error01= y252(it,:)'- H_1 * xi_pred01 -ct;
    forcast_error10= y252(it,:)'- H_0 * xi_pred10 -ct;
    forcast_error11= y252(it,:)'- H_1 * xi_pred11 -ct;
    
    %[12x12]
    SS00= H_0 * P_pred00 * H_0' +R;               % Conditional variance of forecast error: f_{t|t-1}^({0,0}
    SS01= H_1 * P_pred01 * H_1' +R;
    SS10= H_0 * P_pred10 * H_0' +R;
    SS11= H_1 * P_pred11 * H_1' +R;
    
    %@=======================UPDATING===============================@
    % [4x1]  
    xi_upd00 = xi_pred00 + (P_pred00 * H_0') * (SS00 \ forcast_error00); % xi_{t|t}^{0,0}
    xi_upd01 = xi_pred01 + (P_pred01 * H_1') * (SS01 \ forcast_error01);
    xi_upd10 = xi_pred10 + (P_pred10 * H_0') * (SS10 \ forcast_error10);
    xi_upd11 = xi_pred11 + (P_pred11 * H_1') * (SS11 \ forcast_error11);
    
    % [4x4]
    P_upd00 = (eye(4) - (P_pred00 * H_0') * (SS00 \ H_0 )) * P_pred00; % P_{t|t}^{0,0}
    P_upd01 = (eye(4) - (P_pred01 * H_1') * (SS01 \ H_1 )) * P_pred01;
    P_upd10 = (eye(4) - (P_pred10 * H_0') * (SS10 \ H_0 )) * P_pred10;
    P_upd11 = (eye(4) - (P_pred11 * H_1') * (SS11 \ H_1 )) * P_pred11;
