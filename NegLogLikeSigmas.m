function [output, LL, xi_0, xi_1, P_0, P_1, xi00_pred, xi01_pred, xi10_pred, xi11_pred, RMSE, AIC, BIC, xi_smooth, pr_sm0] = NegLogLikeSigmas(param_vector, y252)
...
sigmas_0 = param_vector(9:12);               % 
mu_xi = param_vector(13:16);
sigmas_1 = param_vector(17:20);

[F, H, R, Q_0, dt] = DNS(ft, mu_xi, lambda, alpha, sigmas_0);
[~, ~, ~, Q_1, ~] = DNS(ft, mu_xi, lambda, alpha, sigmas_1);
T = size(y252,1);
mat = [1, 2, 5, 10, 20, 30];

% Noteboom:: different means of states!
% mu1 = parameter1(5,1);
% mu20 = parameter1(6,1); % Markov-switching slope factor
% mu21 = parameter1(7,1);
% mu3 = parameter1(8,1);
% 
% mu_state0 = [mu1; mu20; mu3];
% mu_state1 = [mu1; mu21; mu3];
...
vecp_prev0 = (eye(16) - kron(F,F)) \ reshape(Q_0,[],1); % Initialize at the unconditional covariance matrix
P_prev0 = [vecp_prev0(1:4,1) vecp_prev0(5:8,1) vecp_prev0(9:12,1) vecp_prev0(13:16,1) ];
vecp_prev1 = (eye(16) - kron(F,F)) \ reshape(Q_1,[],1);
P_prev1 = [vecp_prev1(1:4,1) vecp_prev1(5:8,1) vecp_prev1(9:12,1) vecp_prev1(13:16,1) ];
...
for it = 1:T
    ...
    P_pred00 = F * P_prev0 * F' + Q_0;            % P_{t|t-1}^{0,0}
    P_pred01 = F * P_prev0 * F' + Q_1; 
    P_pred10 = F * P_prev1 * F' + Q_0; 
    P_pred11 = F * P_prev1 * F' + Q_1; 
    
    %[12x1]     
    forcast_error00= y252(it,:)'- H * xi_pred00;     % Conditional forecast error: eta_{t|t-1}^{0,0}
    forcast_error01= y252(it,:)'- H * xi_pred01;
    forcast_error10= y252(it,:)'- H * xi_pred10;
    forcast_error11= y252(it,:)'- H * xi_pred11;
    ...
    P_upd00 = (eye(4) - (P_pred00 * H' /(SS00))*H)*P_pred00;
    ...
    P00_pred(:,:,it) = F * P_prev0 * F' + Q_0;            % P_{t|t-1}^{0,0}
    P01_pred(:,:,it) = F * P_prev0 * F' + Q_1; 
    P10_pred(:,:,it) = F * P_prev1 * F' + Q_0; 
    P11_pred(:,:,it) = F * P_prev1 * F' + Q_1; 
